...
class AudioLoader
{
    ...
    public static SoundData CreateSoundData(string fullName, AudioClip audioClip)
    {
        ...
        BGMData soundData = ScriptableObject.CreateInstance<BGMData>();
        ...
        else
        {
            soundData.clip = audioClip;
            soundData.loop = 0;
            soundData.allowMultiple = true;
            soundData.skipIfPlaying = false;
            soundData.important = true;
            soundData.alwaysPlay = true;
            soundData.fadeAtStart = false;
            soundData.delay = 0;
            soundData.startAt = 0;
            soundData.fadeLength = 0;
            soundData.type = SoundData.Type.Default;
            soundData.spatial = 1.0f;
            soundData.pitch = 0.89f;
            soundData.randomPitch = 0;
            soundData.chance = 1;
            soundData.reverbMix = 1;
            soundData.minInterval = 0;
            ...
            BGMData goodData = EClass.Sound.GetData($"Instrument/violin_chaconne") as BGMData;
            soundData.song = goodData.song;
        }

        Plugin.ModLog($"Created sound data: {soundData.name}");
        return soundData;
    }
        ...
        using (UnityWebRequest uwr = UnityWebRequestMultimedia.GetAudioClip(path, type))
        {
            ...
            try
            {
                ...
                if (uwr.result != UnityWebRequest.Result.Success)
                    Plugin.ModLog($"Failed to load WAV AudioClip from path: {path} Full error: {uwr.error}", PrivateLogLevel.Error);
                else
                {
            ...
            {
                Plugin.ModLog($"{err.Message}, {err.StackTrace}", PrivateLogLevel.Error);
            }
        }
        return clip;
    }
    ...
    public static void DumpSoundData(SoundData __result)
    {
        ...
        Plugin.ModLog($"\tSoundData :{__result}");
...
{
    static List<string> customSoundNames = new List<string> { "Instrument/" };
    [HarmonyPrefix]
    ...
    {
        Plugin.ModLog($"Getting sound data: {id}", PrivateLogLevel.Debug);
        // If the sound is a custom sound, return the custom sound data
        if (id != null && id.Contains("custom_"))
        {
            foreach (string prefix in customSoundNames)
            {
            ...
            Plugin.ModLog($"Getting custom sound data: {id}", PrivateLogLevel.Debug);
